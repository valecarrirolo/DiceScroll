...
interface DiceScrollDao {
  ...
  @Insert(onConflict = OnConflictStrategy.REPLACE) fun insertRoll(roll: RollResultEntity)

  @Insert(onConflict = OnConflictStrategy.REPLACE)
  fun insertDieRolls(rolls: List<SingleDieRollEntity>)

  @Insert(onConflict = OnConflictStrategy.REPLACE) fun setTrayDice(dice: List<TrayDieEntity>)

  @Insert(onConflict = OnConflictStrategy.REPLACE) fun setModifier(appState: AppStateEntity)

  @Query("DELETE FROM tray_dice") fun clearTrayDice()

  @Query("DELETE FROM roll_results") fun clearRollHistory()
}
